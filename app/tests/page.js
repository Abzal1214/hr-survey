...
export default function TrainingsPage() {
  ...
  const [newTraining, setNewTraining] = useState({ title: '', description: '', date: '', time: '', location: '', maxParticipants: 20, department: '' });
        ...
        {trainings.length === 0 ? (
          ...
          <div className="space-y-4">
            {trainings.map((t) => {
              ...
              return (
                <div key={tid} className={`rounded-[24px] bg-white/95 p-6 shadow-lg border-l-4 ${past ? 'border-slate-300 opacity-70' : signed ? 'border-emerald-400' : 'border-sky-400'}`}>
